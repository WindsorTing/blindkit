...
# -*- coding: utf-8 -*-
"""
BlindKit v3.0 — Integrated two-root blinding toolkit with comprehensive audit logging
and legacy-aware physiology planning.

What’s new in v3.0 (vs v2.1.1)
- Physiology planner accepts legacy assignment files (CSV/JSON) via --legacy-csv/--legacy-json
  and can include legacy-only animal IDs with --allow-unregistered.
- Planner fills remaining animals to achieve the most even A/B split allowed by legacy and logs
  final counts and whether a perfect 50/50 split was possible.
...
- Physiology planning (50/50 cohort; now legacy-aware).
...
def cmd_init_dual(a):
    ...
    (br / "study_meta.json").write_text(json.dumps({"study_id": a.study_id, "role":"BLINDER","created": iso_now()}, indent=2))
    (er / "study_meta.json").write_text(json.dumps({"study_id": a.study_id, "role":"EXPERIMENTER","created": iso_now()}, indent=2))
    (br / "labels" / "registry.json").write_text(json.dumps({"entries":[]}, indent=2))
    _audit_write(br, "init-dual", study_id=a.study_id, peer_experimenter_root=str(er))
    _audit_write(er, "init-dual", study_id=a.study_id, peer_blinder_root=str(br))
...
def cmd_package_unblinding(a):
    ...
    with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
        ...
        # Summary report
        summary = io.StringIO()
        summary.write("# BlindKit v3.0 — Unblinding Bundle Summary\n")
...
# ---------- CLI ----------
def main():
    ap = argparse.ArgumentParser(prog="blindkit_v3_0", description="Two‑root blinding toolkit + post‑hoc packaging (v3.0)")
